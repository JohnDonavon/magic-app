...
export default function SearchScreen() {
  ...
  const handleCollect = async (card: ScryfallCard, event: any) => {
    event.stopPropagation();
    try {
      const now = Date.now();
      ...
      await CardsTable.insert(cardToSave);
  ...
  const CollectButton = ({ onPress, isCollected }: { onPress: (event: any) => void, isCollected: boolean }) => (
    <Pressable style={[styles.collectButton, isCollected && styles.collectedButton]} onPress={onPress}>
      <Svg viewBox="0 0 24 24" width="16" height="16">
        <Path
          fill={isCollected ? "#4CAF50" : "#343242"}
            ...
            renderItem={({ item }) => (
              <TouchableWithoutFeedback onPress={() => handleCardPress(item)}>
                <View style={styles.cardContainer}>
                  ...
                  <CollectButton 
                    onPress={(e) => handleCollect(item, e)}
                    isCollected={collectedCards.has(item.id)}
                  />
                </View>
