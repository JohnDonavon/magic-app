...
//   //   this.db = await SQLite.openDatabaseAsync('mtg_overseer.db');
...
//     return await this.db.getFirstAsync<Card>('SELECT * FROM cards WHERE id = ?', [id]);
//   }

//   async getAllCards(): Promise<Card[]> {
//     return await this.db.getAllAsync<Card>('SELECT * FROM cards');
//   }

//   async insertDeck(deck: Deck): Promise<void> {
//     await this.db.runAsync(
//       'INSERT INTO decks (id, name, description, format, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
//       [deck.id, deck.name, deck.description || null, deck.format || null, deck.created_at, deck.updated_at]
//     );
//   }

//   async insertDeckCard(deckCard: DeckCard): Promise<void> {
//     await this.db.runAsync(
...
// export const initDatabase = () => db.initialize();
...
//   try {
//     await db.insertDeckCard(deckCard);
//   } catch (error) {
//     throw error;
//   }
// };
